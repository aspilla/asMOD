...
function asCastingBarMixin:OnLoad(unit, showTradeSkills, showShield)
	self.StagePoints = {};
	self.StagePips = {};
	self.StageTiers = {};
...
function asCastingBarMixin:UpdateShownState(desiredShow)
...
function asCastingBarMixin:AddWidgetForFade(widget)
...
function asCastingBarMixin:SetUnit(unit, showTradeSkills, showShield)
...
function asCastingBarMixin:OnShow()
	if ( self.unit ) then
...
function asCastingBarAnim_OnInterruptSparkAnimFinish(self)
	local castingBar = self:GetParent();
	castingBar:SetValue(castingBar.maxValue);
	castingBar:HideSpark();
end

function asCastingBarAnim_OnFadeOutFinish(self)
	local castingBar = self:GetParent();
	castingBar:Hide();
end

function asCastingBarMixin:GetEffectiveType(isChannel, notInterruptible, isTradeSkill, isEmpowered)
...
function asCastingBarMixin:GetTypeInfo(barType)
	if not barType then
		barType = "standard";
	end
	return asCASTING_BAR_TYPES[barType];
end

function asCastingBarMixin:OnEvent(event, ...)
	local arg1 = ...;
...
function asCastingBarMixin:UpdateInterruptibleState(notInterruptible)
...
function asCastingBarMixin:OnUpdate(elapsed)
...
function asCastingBarMixin:ApplyAlpha(alpha)
	self:SetAlpha(alpha);
...
function asCastingBarMixin:FinishSpell()
...
function asCastingBarMixin:ShowSpark()
	if ( self.Spark ) then
		self.Spark:Show();
	end
	...
	for barType, barTypeInfo in pairs(asCASTING_BAR_TYPES) do
...
function asCastingBarMixin:HideSpark()
	if ( self.Spark ) then
		self.Spark:Hide();
	end
...
function asCastingBarMixin:PlayInterruptAnims()
...
function asCastingBarMixin:StopInterruptAnims()
	if self.HoldFadeOutAnim then
...
function asCastingBarMixin:PlayFadeAnim()
...
function asCastingBarMixin:PlayFinishAnim()
	if not self.playCastFX then
		return;
	end
...
function asCastingBarMixin:StopFinishAnims()
	if self.FlashAnim then
		self.FlashAnim:Stop();
	end
	...
	for _, barTypeInfo in pairs(asCASTING_BAR_TYPES) do
		local finishAnim = barTypeInfo.finishAnim and self[barTypeInfo.finishAnim];
		if finishAnim then
			finishAnim:Stop();
		end
...
function asCastingBarMixin:StopAnims()
	self:StopInterruptAnims();
	self:StopFinishAnims();
end

function asCastingBarMixin:UpdateIsShown()
...
function asCastingBarMixin:SetAndUpdateShowCastbar(showCastbar)
	self.showCastbar = showCastbar;
	self:UpdateIsShown();
end

function asCastingBarMixin:SetLook(look)
...
function asCastingBarMixin:AddStages(numStages)
	...
	for i = 1,self.NumStages-1,1 do
		...
		if(duration > CASTBAR_STAGE_DURATION_INVALID) then
			...
			if not stagePip then
				stagePip = CreateFrame("FRAME", nil, self, hasFX and "asCastingBarFrameStagePipFXTemplate" or "asCastingBarFrameStagePipTemplate");
...
function asCastingBarMixin:UpdateStage()
	local maxStage = 0;
...
function asCastingBarMixin:ClearStages()
...
asTargetSpellBarMixin = CreateFromMixins(asCastingBarMixin);

function asTargetSpellBarMixin:OnEvent(event, ...)
	local arg1 = ...
	...
	asCastingBarMixin.OnEvent(self, event, arg1, select(2, ...));
end

LoadAddOn("asMOD");
function asTargetSpellBarMixin:AdjustPosition()
