...
function ns.update_unitframe_other(frame)
    ...
    if frame.isplayerframe or frame.istargetframe then
        if UnitAffectingCombat(unit) then
            combattext = combattext .. icons.combat;
        end

        if IsResting() and frame.isplayerframe then
            combattext = combattext .. icons.resting;
        end

        frame.combattext:SetText(combattext);
    end
    --Power
    ...
    if powerType ~= nil then
        ...
        if frame.isplayerframe then
            if powerType > 0 or (ASPowerBarInfo and ASPowerBarInfo.nomana) then
