...
local function getAura(id)
    ...
    if real then
        if type(real) == "table" then
            for _, realid in pairs(real) do
				local aura = C_UnitAuras.GetPlayerAuraBySpellID(realid) ;
				if aura then
                    return aura;
                end
            end
	...
	return C_UnitAuras.GetPlayerAuraBySpellID(id);
end
