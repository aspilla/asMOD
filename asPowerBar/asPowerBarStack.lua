...
local function create_aurabutton(max, color, showcount)
...
local function setup_container(spellid, max, color, unit, showcount)
    ...
    if main_frame.container == nil then
        ...
        add_group(main_frame.container, "stack", filter, cfilters,
            { maxFrameCount = 1, initializeFrame = create_aurabutton(max, color, showcount) });
...
function ns.setup_stack(stackinfo)
    ...
    ns.bar:SetValue(0);
    ns.bar.text:Hide();
    setup_container(stackinfo.spellid, stackinfo.max, stackinfo.color, stackinfo.unit, stackinfo.showcount);
