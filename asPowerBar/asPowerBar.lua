...
local function set_height(heightprimary, heightsecondary)
    ...
    for i = 1, 20 do
        ns.bar.countframes[i]:SetHeight(heightprimary + 2);
    end
end
...
local function init_class()
    ...
    if (englishClass == "MAGE") then
        ...
        if (spec and spec == 3) then
            auraid = 205473;
            max_aura = 5;
            stackinfo = { spellid = 1221389, max = 20, color = CreateColor(0.7, 0.3, 1), unit = "target", showcount = true }
        end
    end
    ...
    if (englishClass == "WARLOCK") then
        ...
        if (spec and spec == 2) then
            bsmallprimary = true;
            bbigsecondary = true;
            stackinfo = { spellid = 264173, max = 4, color = CreateColor(0.7, 0.3, 1), unit = "player", showcount = false }
        end
        if (spec and spec == 3) then
            bpartial = true;
            bsmallprimary = true;
