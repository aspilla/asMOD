...
local function setup_frames()
    ...
    if ns.options.ShowTarget then
        main_frame.targetanchor = CreateFrame("Frame", nil, UIParent);
        main_frame.targetanchor:SetPoint("LEFT", UIParent, "CENTER", ns.configs.target_xpoint,
            ns.configs.target_ypoint - offset);
        main_frame.targetanchor:SetWidth(1);
        main_frame.targetanchor:SetHeight(1);
        main_frame.targetanchor:Show();
        if libasConfig then
            libasConfig.load_position(main_frame.targetanchor, "asDebuffFilter(Target)", ADF_Positions_1);
        end
        ...
        main_frame.targetframe:SetPoint("LEFT", main_frame.targetanchor, "LEFT", 0, 0);
    ...
    if ns.options.ShowPlayer then
        ...
        if libasConfig then
            libasConfig.load_position(main_frame.playeranchor, "asDebuffFilter(Player)", ADF_Positions_2);
        end
        local cfilter = {}
    ...
    if ns.options.ShowPrivate2 then
        main_frame.private_frame = CreateFrame("Frame", nil, main_frame)

        main_frame.private_frame:SetPoint("RIGHT", UIParent, "CENTER", ns.configs.private_xpoint,
            ns.configs.private_ypoint - offset)
