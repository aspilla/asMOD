...
local function show_interruptspell(frame, spellid, alpha, unit)
	...
	if ns.options.CheckRange then
		outrange = ns.check_range(unit, spellid);
	end
	...
	if (isUsable) then
		if outrange then
			frame.icon:SetVertexColor(1.0, 0, 0);
		else
...
local function find_spell(isboss)
	...
	do
		...
		if interrupt_spellid == nil and not isboss and ns.options.ShowStunOnInterrupt then
		...
		if interrupt_spellid == nil then
			interrupt_spellid = firstspellid;
		end
	end
	...
	if not isboss then
		...
		if stun_spellid == nil then
			stun_spellid = firstspellid;
		end
	end
...
local function check_casting(frame, unit)
	...
	if name then
		...
		if interrupt_spellid then
			show_interruptspell(frame.ibutton, interrupt_spellid, 0, unit);
		else
			frame.ibutton:Hide();
		end

		if stun_spellid then
			show_interruptspell(frame.sbutton, stun_spellid, 1, unit);
		else
			frame.sbutton:Hide();
		end
