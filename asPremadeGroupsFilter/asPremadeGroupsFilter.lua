...
local rangedspecs = {
	["EVOKERDevastation"] = true,
	["EVOKERAugmentation"] = true,
	["DRUIDBalance"] = true,
	["MAGEArcane"] = true,
	["MAGEFire"] = true,
	["MAGEFrost"] = true,
	["HUNTERBeast Mastery"] = true,
	["HUNTERMarksmanship"] = true,
	["PRIESTShadow"] = true,
	["DEMONHUNTERDevourer"] = true,
	["SHAMANElemental"] = true,
	["WARLOCKAffliction"] = true,
	["WARLOCKDemonology"] = true,
	["WARLOCKDestruction"] = true,
}
...
roleicons["TANK"] = CreateAtlasMarkup("UI-LFG-RoleIcon-Tank", 16, 16, 0, 0);
roleicons["DAMAGER"] = CreateAtlasMarkup("UI-LFG-RoleIcon-Dps", 16, 16, 0, 0);
roleicons["RANGER"] = CreateAtlasMarkup("UI-LFG-RoleIcon-RangedDPS", 16, 16, 0, 0);
...
local function setupText(role, infos, fontstring)
	...
	if total > 0 and (role == "DAMAGER" or role == "RANGER") then
		text = total .. text;
	end
...
local function update_searchentry(entry, ...)
	...
	elseif (categoryID == 3) then
		...
		for i = 1, numMembers do
			local role, class, classLocalized, specLocalized, isLeader = C_LFGList.GetSearchResultMemberInfo(resultID, i);
			local isrange = rangedspecs[class .. specLocalized];
		...
		if not entry.DataDisplay.RoleCount.infos then
			...
			entry.DataDisplay.RoleCount.infos.RANGER = entry.DataDisplay.RoleCount:CreateFontString(nil, "ARTWORK");
			entry.DataDisplay.RoleCount.infos.RANGER:SetFont(STANDARD_TEXT_FONT, 12);
			entry.DataDisplay.RoleCount.infos.RANGER:SetPoint("RIGHT", entry.DataDisplay.RoleCount.HealerIcon,
				"BOTTOMRIGHT", 40, -10);
			...
			entry.DataDisplay.RoleCount.infos.DAMAGER:SetPoint("RIGHT", entry.DataDisplay.RoleCount.infos.RANGER,
				"LEFT", -5, 0);
