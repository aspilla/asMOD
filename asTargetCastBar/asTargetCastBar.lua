...
local ATCB_HEIGHT = 15
local ATCB_X = 0;
local ATCB_Y = 0;
local ATCB_ALPHA = 0.8;
local ATCB_NAME_SIZE = 14;
local ATCB_TIME_SIZE = 10;
...
ATCB.castbar:SetWidth(ATCB_WIDTH)
...
ATCB.castbar.name:SetPoint("CENTER", ATCB.castbar, "CENTER", 0, 0);
...
ATCB.castbar:SetPoint("CENTER",UIParent,"CENTER", ATCB_X, ATCB_Y)
ATCB.castbar:Hide();

ATCB.button = CreateFrame("Button", "ATCBIcon", ATCB.castbar, "ATCBFrameTemplate");
ATCB.button:SetPoint("RIGHT", ATCB.castbar,"LEFT", -2, 0)
ATCB.button:SetWidth(ATCB_HEIGHT * 1.2);
ATCB.button:SetHeight(ATCB_HEIGHT);
	...
	if event == "PLAYER_TARGET_CHANGED" or event == "PLAYER_ENTERING_WORLD" then
		...
		if name then
			...
			if notInterruptible then
				castBar:SetStatusBarColor(0.1, 0.1, 0.1);
			else
				castBar:SetStatusBarColor(0, 0.9, 0);
			end
			text:SetText(name);
