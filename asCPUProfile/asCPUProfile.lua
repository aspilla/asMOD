...
content:SetSize(550, 800) -- 충분히 큰 높이로 설정
scrollFrame:SetScrollChild(content)

-- 필드 제목(헤더) 행 생성
local header = CreateFrame("Frame", nil, content)
header:SetSize(550, 20)
header:SetPoint("TOPLEFT", 0, 0)

-- 필드 제목 텍스트 설정
local headers = { "Addon Name", "PeakTime", "BossAvg", "Over5Ms", "Over10Ms", "Over50Ms" }
local headerWidths = { 150, 80, 80, 80, 80, 80 }

for i, title in ipairs(headers) do
    local headerText = header:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
    headerText:SetPoint("LEFT", header, "LEFT", (i - 1) * headerWidths[i], 0)
...
local function CreateOrUpdateRow(parent, addonName, cpuUsages)
    local rowInfo = rowList[addonName]

    if rowInfo then
        -- 행이 이미 존재하면 업데이트
        for i = 1, 5 do
            if i <= 2 then
                rowInfo.cpuTexts[i]:SetText(string.format("%.4f", cpuUsages[i] or 0))
            else
                rowInfo.cpuTexts[i]:SetText(string.format("%.0f", cpuUsages[i] or 0))
            end
        end
    else
        -- 행이 존재하지 않으면 새로 생성
        ...
        for i = 1, 5 do
            ...
            cpuText:SetPoint("LEFT", nameText, "RIGHT", (i - 1) * 80, 0) -- 각 필드를 오른쪽으로 나열
            cpuText:SetWidth(80)
            cpuText:SetJustifyH("CENTER")
            if i <= 2 then
                cpuText:SetText(string.format("%.4f", cpuUsages[i] or 0))
            else
                cpuText:SetText(string.format("%.0f", cpuUsages[i] or 0))
            end
            cpuTexts[i] = cpuText
        end
...
local function checkAddon(name)
    ...
    if beforecombat[name] then
        Over5Ms = Over5Ms - beforecombat[name][3];
        Over10Ms = Over10Ms - beforecombat[name][4];
        Over50Ms = Over50Ms - beforecombat[name][5];
    end
    ...
    CreateOrUpdateRow(content, name, cpus);
end
