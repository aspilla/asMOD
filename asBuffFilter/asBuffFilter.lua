...
local function setup_frames()
    ...
    if libasConfig then
        libasConfig.load_position(main_frame.targetanchor, "asBuffFilter(Target)", ABF_Positions);
    end
    ...
    main_frame.helpfulframe:SetPoint("LEFT", main_frame.targetanchor, "LEFT", 0, 0);
    ...
    main_frame.nchelpfulframe:SetPoint("LEFT", main_frame.targetanchor, "LEFT", 0, 0);
    ...
    add_group(main_frame.harmfulframe, "buffs", filters.harmful, { isStealable = false },
        { maxFrameCount = ns.configs.combat_max_buffs, initializeFrame = create_aurabutton(ns.configs.size, false) });
    main_frame.harmfulframe:SetPoint("LEFT", main_frame.targetanchor, "LEFT", 0, 0);
